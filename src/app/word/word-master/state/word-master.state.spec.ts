...
describe('App actions', () => {
  ...
  it('should create an action and add an item', () => {
    store.dispatch(new AddedWord({ word: 'test', id: null, starred: false, definitions: null }));
    store.select(state => state.wordmaster.words).subscribe((words: string[]) => {
      expect(words).toEqual(jasmine.objectContaining([{ word: 'test', id: null, starred: false, definitions: null }]));
    });
  });
